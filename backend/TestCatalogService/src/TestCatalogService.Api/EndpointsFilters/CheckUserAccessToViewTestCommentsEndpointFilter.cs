...
public class CheckUserAccessToViewTestCommentsEndpointFilter : IEndpointFilter
{
    ...
    public CheckUserAccessToViewTestCommentsEndpointFilter(
    ...
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next) {
        ...
        if (userIdRes.IsSuccess(out var userId)) {
            access = await test.CheckUserAccessToViewTestComments(userId, _userFollowingsRepository.GetUserFollowings);
        }
        else {
            access = test.CheckAccessToViewTestCommentsForUnauthorized();
        }
