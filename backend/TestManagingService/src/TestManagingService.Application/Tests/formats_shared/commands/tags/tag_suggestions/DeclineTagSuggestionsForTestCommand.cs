...
public record class DeclineTagSuggestionsForTestCommand(
    TestId TestId,
    HashSet<TestTagId> Tags
) : IRequest<ErrOrNothing>;

internal class DeclineTagSuggestionsForTestCommandHandler
    : IRequestHandler<DeclineTagSuggestionsForTestCommand, ErrOrNothing>

{
    private readonly IBaseTestsRepository _baseTestsRepository;

    public DeclineTagSuggestionsForTestCommandHandler(IBaseTestsRepository baseTestsRepository) {
        _baseTestsRepository = baseTestsRepository;
    }

    public async Task<ErrOrNothing> Handle(
        DeclineTagSuggestionsForTestCommand request,
        ...
        var res = test.DeclineTagSuggestions(request.Tags);
