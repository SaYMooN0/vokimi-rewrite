...
public class GeneralTestQuestion : AggregateRoot<GeneralTestQuestionId>
{
    ...
    protected List<GeneralTestAnswer> _answers { get; init; }
    ...
    public ErrOrNothing AddNewAnswer(
        ...
        if (answerData.MatchingEnumType != AnswersType) {
            return new Err(
                ...
                details:
                $"Question has answers type {AnswersType} and provided data is of type {answerData.MatchingEnumType}"
            );
        }
        ...
        if (_answers.Count >= GeneralTestQuestionRules.MaxAnswersCount) {
            return new Err(
                ...
                details:
                $"Maximum number of answers allowed is {GeneralTestQuestionRules.MaxAnswersCount}. Question already has {_answers.Count} answers"
            );
        }
    ...
    public ErrOrNothing RemoveAnswer(GeneralTestAnswerId answerId) {
        ...
        if (answer is null) {
            return Err.ErrFactory.NotFound(
                ...
                details:
                $"General test question with id {Id} has no answer with id {answerId}. Either it has already been removed or it was never in the question"
            );
        }
